...
public class CrudServiceProd_Prov implements ICrudServiceProd_Prov {
	...
	public List<Prod_ProvDto> findAllREST(String search) throws Exception {
		String url = "http://localhost:8080/prod_prov/REST";
		...
		ResponseEntity<Prod_ProvDto[]> responseEntity = new RestTemplate().exchange(
				url, HttpMethod.GET, new HttpEntity<>(headers), Prod_ProvDto[].class);
	...
	public Optional<Prod_ProvDto> findByIdREST(int id) throws Exception {
		Prod_ProvDto dto = null;

		ResponseEntity<Prod_ProvDto> responseEntity = new RestTemplate().exchange(
				"http://localhost:8080/prod_prov/REST/" + id,
				HttpMethod.GET, new HttpEntity<>(httpHeaders()), new ParameterizedTypeReference<Prod_ProvDto>() {
				});
	...
	@Override
	public Prod_ProvDto editarREST(Prod_ProvDto p) throws Exception {
		Prod_ProvDto dto = null;

		ResponseEntity<Prod_ProvDto> responseEntity = new RestTemplate().exchange("http://localhost:8080/prod_prov/REST",
				HttpMethod.PUT, new HttpEntity<>(p, httpHeaders()), new ParameterizedTypeReference<Prod_ProvDto>() {
				});
	...
	@Override
	public Prod_ProvDto saveREST(Prod_ProvDto p) throws Exception {
		Prod_ProvDto dto = null;

		ResponseEntity<Prod_ProvDto> responseEntity = new RestTemplate().exchange("http://localhost:8080/prod_prov/REST",
				HttpMethod.POST, new HttpEntity<>(p, httpHeaders()), new ParameterizedTypeReference<Prod_ProvDto>() {
				});
	...
	public Prod_ProvDto deleteREST(int id) throws Exception {
		...
		ResponseEntity<Prod_ProvDto> responseEntity = new RestTemplate().exchange(
				"http://localhost:8080/prod_prov/REST/" + id,
				HttpMethod.DELETE, new HttpEntity<>(httpHeaders()), new ParameterizedTypeReference<Prod_ProvDto>() {
				});
